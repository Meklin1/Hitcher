...
public class HomeController : Controller
{
    ...
    //Using dependency injection for UserModel
    {
        ...
        _driverList = _driverRepository.GetDriverList();
    }
    ...
    public IActionResult DriverProfile(string message)
    {
        ViewBag.Message = message;
        if (TempData["DriverModel"] is not null)
        {
            DriverModel user = JsonConvert.DeserializeObject<DriverModel>((string)TempData["DriverModel"]);
            return View("DriverProfile", user);
        }
        else
        {
            // return View(new UserModel());
            ...
            var stringId = HttpContext.Session.GetString(LoggedUser);
            Guid id = Guid.Parse(stringId);
            return View(_driverRepository.GetDriver(id));
        }
    }
    ...
    public IActionResult AddDriver(DriverModel driver)
    {
        ...
        var userID = HttpContext.Session.GetString(LoggedUser);
        driver.UserID = Guid.Parse(userID);
        if (!_driverList.Any(driver => driver.UserID == driver.UserID))
        {
    ...
    public IActionResult AddTravel(FormInput input)
    {
        ...
        var bearingList = input.Bearings.Split(',').Select(word => Convert.ToDouble(word, CultureInfo.InvariantCulture)).ToList();
        var distanceList = input.Distance.Split(',').Select(word => Convert.ToDouble(word, CultureInfo.InvariantCulture)).ToList();
        var latList = new List<double>(Array.ConvertAll(input.Lat.Split(","), s => Double.Parse(s, CultureInfo.InvariantCulture)));
        var lngList = new List<double>(Array.ConvertAll(input.Lng.Split(","), s => Double.Parse(s, CultureInfo.InvariantCulture)));
